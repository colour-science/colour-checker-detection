...
APPLICATION_NAME: str = colour_checker_detection.__application_name__

APPLICATION_VERSION: str = colour_checker_detection.__version__
...
def docs(ctx: Context, html: Boolean = True, pdf: Boolean = True):
    ...
    with ctx.prefix('export COLOUR_SCIENCE__DOCUMENTATION_BUILD:Boolean=True'):
        with ctx.cd('docs'):
            if html:
...
def requirements(ctx: Context):
    ...
    ctx.run('poetry run pip list --format=freeze | '
            'egrep -v "colour-checker-detection=" '
            '> requirements.txt')
